...
def save_dataframes_to_db(participating_client_df, clean_demo_df, session_deduplicated_web_data_df):
    ...
    # Upload tables
    participating_client_df.to_sql("participants", con = engine, if_exists="fail", index=False)
    clean_demo_df.to_sql("clients", con = engine, if_exists="fail", index=False)
    session_deduplicated_web_data_df.to_sql("session", con = engine, if_exists="fail", index=False)
